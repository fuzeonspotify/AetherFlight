...
def log(message: str) -> None:
    unreal.log(f"[Aether UE5.8 World Repair v3] {message}")
...
def missing_weight(material, x: int, y: int):
    """Return 1 - saturate(sum of all four painted layer weights)."""
    ...
    saturated = expression(material, unreal.MaterialExpressionSaturate, x + 1160, y)
    connect(total, "", saturated, "Input", "total Landscape weight to Saturate")
    inverse = expression(material, unreal.MaterialExpressionOneMinus, x + 1380, y)
    connect(saturated, "", inverse, "Input", "saturated weight to OneMinus")
    return inverse
