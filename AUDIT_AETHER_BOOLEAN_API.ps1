...
$script = Join-Path $projectRoot "Content\Python\AuditAetherBooleanModifierAPI_UE58.py"
$report = Join-Path $projectRoot "Saved\AetherBooleanModifierAPIAudit.txt"

if (Get-Process UnrealEditor -ErrorAction SilentlyContinue) {
    throw "Close every Unreal Editor and Crash Reporter window before running the Boolean API audit."
}
...
if (!(Test-Path -LiteralPath $script)) {
    throw "Boolean API audit script was not found: $script"
}

Remove-Item -LiteralPath $report -Force -ErrorAction SilentlyContinue

Write-Host ""
Write-Host "Checking the exact UE 5.8 Mesh Terrain Boolean API exposed by this installation..."
...
if ($reportText -notmatch "AETHER_BOOLEAN_API=PASS") {
    throw "The Boolean API needs inspection before installing the cave stage. Paste the complete report shown above."
}
